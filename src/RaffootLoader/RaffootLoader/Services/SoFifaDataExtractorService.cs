...
namespace RaffootLoader.Services
{
    public class SoFifaDataExtractorService
    {
        ...
        private readonly IEnumerable<League> _leagues = new[]
        {
        ...
        {
            foreach (var group in _leagues.GroupBy(l => l.ExternalId))
            {
                if (group.Count() > 1)
                    throw new Exception();
            }
            ...
            foreach (var league in _leagues)
            {
                ...
                await GetClubs(league).ConfigureAwait(false);
                Console.WriteLine();
            }
        }

        private async Task GetClubs(League league)
        {
            ...
            foreach (var tr in rows)
            {
                ...
                await GetPlayers(club).ConfigureAwait(false);
